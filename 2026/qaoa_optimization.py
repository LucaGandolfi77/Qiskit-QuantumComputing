...
# Initialize sampler (deferred QAOA attempt)
sampler = None
...
# We'll attempt QAOA later only if Hamiltonian is not trivially diagonal
# and sampler + QAOA implementation are available. For diagonal Hamiltonians
# we use the fast classical exact evaluation below to avoid constructing
# huge dense matrices.
...
# Decide approach: diagonal fast-path, else QAOA (if available), else sparse eigensolver
result = None
try:
    if is_diagonal_in_z(hamiltonian):
        ...
        # attempt QAOA only for non-diagonal Hamiltonians
        if QAOA is not None and COBYLA is not None and sampler is not None:
            ...
            print("QAOA or sampler not available; attempting sparse eigensolver.")
...
if hasattr(result, 'eigenvalue'):
    try:
        ...
        print(f"Optimal value (including constant): {getattr(result, 'eigenvalue', None)}")
else:
    print('No eigenvalue available in result')
if hasattr(result, 'optimal_parameters'):
    print(f"Optimal parameters: {result.optimal_parameters}")
else:
    print('Optimal parameters: N/A')

# Extract the most likely solution from result

def extract_bitstring_from_result(res, n_qubits):
    # Priority: best_measurement -> eigenstate probabilities
    if res is None:
        return None
    if hasattr(res, 'best_measurement'):
        bm = res.best_measurement
        return bm.get('bitstring') if isinstance(bm, dict) else None
    if hasattr(res, 'eigenstate'):
        es = res.eigenstate
        try:
            vec = None
            # try numpy array
            import numpy as _np
            if isinstance(es, _np.ndarray):
                vec = es
            else:
                # Try to extract underlying data if Statevector-like
                try:
                    vec = _np.asarray(es)
                except Exception:
                    # try .data or .to_statevector()
                    try:
                        vec = _np.asarray(es.data)
                    except Exception:
                        vec = None
            if vec is not None:
                probs = _np.abs(vec)**2
                idx = int(_np.argmax(probs))
                # convert index to bitstring with qubit i as bit i (LSB first)
                bits = ''.join(str((idx >> i) & 1) for i in range(n_qubits))
                return bits
        except Exception:
            return None
    return None

# Get the solution bitstring
solution_bitstring = extract_bitstring_from_result(result, num_qubits)
