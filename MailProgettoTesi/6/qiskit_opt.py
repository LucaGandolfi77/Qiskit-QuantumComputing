...
def safe_vm_alloc(capacities: list[int], n_vms: int, margin: float = 1.25) -> list[int]:
    """
    Only need to satisfy server_load now (first3vm constraint removed).
    sum(lims) >= sum(caps) - n_servers
    """
    n_s = len(capacities)
    need_load = max(sum(capacities) - n_s, 0)
    min_per_vm = math.ceil(need_load / n_vms)
...
if args.vm_allocation_limits:
    ...
    vm_allocation_limits = safe_vm_alloc(capacities, n_vms, n_servers)
