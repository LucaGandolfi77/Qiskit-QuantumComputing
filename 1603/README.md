...
### Opzioni GPU (CuPy / cuQuantum)

Se vuoi sfruttare la GPU per accelerare algebra lineare e simulazione statevector, installa CuPy e (opzionalmente) il pacchetto NVIDIA cuQuantum. Su Windows è consigliato usare WSL2 o un sistema Linux con CUDA installato.

Esempio (CUDA 11.8, pip):

```bash
# crea/attiva ambiente (consigliato usare conda su Linux per gestire CUDA)
python -m venv .venv
source .venv/bin/activate

# Installa CuPy per CUDA 11.8 (scegli la variante corretta per la tua CUDA)
pip install cupy-cuda118

# (opzionale) NVIDIA cuQuantum per accelerazioni specifiche sul simulatore
pip install cuquantum

# Reinstallare/ricostruire qiskit-aer con supporto GPU se necessario
pip install qiskit-aer
```

Se usi conda (consigliato su Linux):

```bash
conda create -n qaoa python=3.11 -y
conda activate qaoa
conda install -c conda-forge cupy cudatoolkit=11.8
pip install cuquantum qiskit-aer
```

Test rapido GPU / CuPy:
...
Test integrazione project wrapper:

```bash
python -c "import cupy_utils; print('cupy available:', cupy_utils.has_cupy())"
```

Note pratiche:
- Verifica la versione di CUDA sulla macchina e installa la build di CuPy corrispondente (es. `cupy-cuda117`, `cupy-cuda118`, ...).\
- Su Windows preferibile usare WSL2 con GPU passthrough o un nodo Linux con driver NVIDIA aggiornati.\
- L'accelerazione reale dipende dalla compatibilità delle librerie (SciPy sparse vs cupyx.sparse). Alcune conversioni sparse rimangono costose.
