...
# uj has lb = min_cpu_per_vm (≥1) to stay away from 0
...
# Server load: sum_j vj >= cap[i] - 1 - small_slack
load_slack = 0.0  # puoi mettere 0.1 se vuoi meno soluzioni “sul bordo”
for i in range(n_servers):
    mdl.add_constraint(
        mdl.sum(v_vars[(j, i)] for j in range(n_vms))
        >= capacities[i] - 1 - load_slack,
        f"cons_server_load_{i}",
    )

# Nessun vincolo "first 3 VMs" qui.
