"""run_and_compare_gui_fixed.py

GUI tool to select a CSV (or JSON) input file, run Gurobi + (optional) QAOA, and generate an HTML report.

Fixes:
- Removes external imports (model_runner/html_generator). Everything is self-contained.
- Avoids invalid f-string format specifiers by pre-formatting numeric values.

Usage:
  python 2026/run_and_compare_gui_fixed.py

Input formats:
- JSON: expected to be a dict like {"problem_name": {"variables_config": {...}, "objective_coefficients": {...}}, ...}
  (same schema used in previous JSON-based scripts).
- CSV: expects a notebook-style CSV with columns [notebook, section, content] (like all_qiskit_lp.csv).
  Parsing is best-effort: binaries + maximize are extracted; constraints are ignored for now.

Outputs:
- HTML report and JSON results file in chosen output directory.
"""
...
class GurobiSolver:
    def solve(self, problem_data: dict) -> dict:
        ...
        if not var_cfg or not obj_coeffs:
            return {'status': 'error', 'time': time.time() - start, 'error': 'Empty variables_config/objective_coefficients'}

        try:
            mdl = gp.Model("gurobi_model")
            ...
            obj_expr = gp.quicksum(float(obj_coeffs.get(v, 0.0)) * variables[v] for v in variables)
...
class QAOASolver:
    def solve(self, problem_data: dict) -> dict:
        ...
        try:
            ...
            linear = {v: -float(obj_coeffs.get(v, 0.0)) for v in bin_vars}
...
class ModelRunner:
    ...
    def run_all(self) -> list:
        ...
        for i, (name, pdata) in enumerate(items, 1):
            ...
            print("✓" if g.get('status') == 'optimal' else "✗")
            result['gurobi'] = g

            print("  🟣 QAOA...", end=' ', flush=True)
            q = self.qaoa.solve(pdata)
            print("✓" if q.get('status') == 'completed' else "-" if q.get('status') == 'skipped' else "✗")
            result['qaoa'] = q
...
<div class="model-card">
  <div class="model-header">
    <h2>📊 {result.get('model_name','Unknown')}</h2>
  </div>
  ...
  <table>
    <tr><th>Metodo</th><th>Status</th><th>Obiettivo</th><th>Tempo (s)</th><th>Soluzioni</th><th>Preview</th></tr>
    <tr class="{g_winner}">
...
def load_problems_from_json(json_path: Path) -> dict:
    ...
    if isinstance(data, list):
        ...
        for m in data:
            ...
            problems[name] = {
                'variables_config': m.get('variables', {}),
                'objective_coefficients': m.get('objective_coefficients', {}),
...
class App:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Gurobi vs QAOA - Comparator")
    ...
    def _build_ui(self):
        header = tk.Frame(self.root, bg='#667eea', height=90)
        header.pack(fill='x')

        tk.Label(header, text="🔬 Gurobi vs QAOA Comparator", font=('Arial', 18, 'bold'), bg='#667eea', fg='white').pack(pady=(12, 0))
        tk.Label(header, text="Seleziona un CSV (o JSON) e genera un report HTML", font=('Arial', 10), bg='#667eea', fg='white').pack()

        main = tk.Frame(self.root, padx=24, pady=18)
        main.pack(fill='both', expand=True)

        file_box = tk.LabelFrame(main, text="Input file", font=('Arial', 11, 'bold'), padx=10, pady=10)
        file_box.pack(fill='x', pady=(0, 12))

        self.file_lbl = tk.Label(file_box, text="Nessun file selezionato", fg='gray', wraplength=640, justify='left')
        self.file_lbl.pack(anchor='w')

        btns = tk.Frame(file_box)
        btns.pack(pady=8)

        tk.Button(btns, text="📂 Seleziona CSV", command=self._pick_csv, bg='#667eea', fg='white', padx=18, pady=8).pack(side='left', padx=6)
        tk.Button(btns, text="📄 Seleziona JSON", command=self._pick_json, bg='#764ba2', fg='white', padx=18, pady=8).pack(side='left', padx=6)

        out_box = tk.LabelFrame(main, text="Output", font=('Arial', 11, 'bold'), padx=10, pady=10)
        out_box.pack(fill='x', pady=(0, 12))

        self.out_lbl = tk.Label(out_box, text="Default: ./reports", fg='gray', wraplength=640, justify='left')
        self.out_lbl.pack(anchor='w')

        tk.Button(out_box, text="📁 Seleziona cartella output", command=self._pick_out, bg='#3498db', fg='white', padx=18, pady=8).pack(pady=8)

        st_box = tk.LabelFrame(main, text="Stato librerie", font=('Arial', 11, 'bold'), padx=10, pady=10)
        st_box.pack(fill='x', pady=(0, 12))

        g_st = "✅ Available" if GUROBI_AVAILABLE else "❌ Not installed"
        q_st = "✅ Available" if QISKIT_AVAILABLE else "❌ Not installed"
        tk.Label(st_box, text=f"Gurobi: {g_st}", font=('Arial', 10)).pack(anchor='w')
        tk.Label(st_box, text=f"Qiskit: {q_st}", font=('Arial', 10)).pack(anchor='w')

        self.progress_lbl = tk.Label(main, text="Pronto", fg='green')
        self.progress_lbl.pack(pady=(6, 0))

        self.progress = ttk.Progressbar(main, mode='indeterminate', length=660)

        act = tk.Frame(main)
        act.pack(fill='x', pady=(14, 0))

        self.run_btn = tk.Button(act, text="▶️ Esegui", command=self._run, bg='#27ae60', fg='white', font=('Arial', 12, 'bold'), padx=22, pady=12, state='disabled')
        self.run_btn.pack(side='left', expand=True, padx=6)

        tk.Button(act, text="❌ Esci", command=self.root.quit, bg='#e74c3c', fg='white', font=('Arial', 12, 'bold'), padx=22, pady=12).pack(side='left', expand=True, padx=6)

    def _pick_csv(self):
        p = filedialog.askopenfilename(title="Seleziona CSV", filetypes=[("CSV", "*.csv"), ("All", "*.*")], initialdir=Path.cwd())
        if p:
            self.selected_file = Path(p)
            self.file_lbl.config(text=f"Selezionato: {self.selected_file}", fg='green')
            self.run_btn.config(state='normal')

    def _pick_json(self):
        p = filedialog.askopenfilename(title="Seleziona JSON", filetypes=[("JSON", "*.json"), ("All", "*.*")], initialdir=Path.cwd())
        if p:
            self.selected_file = Path(p)
            self.file_lbl.config(text=f"Selezionato: {self.selected_file}", fg='green')
            self.run_btn.config(state='normal')
    ...
    def _pick_out(self):
        p = filedialog.askdirectory(title="Seleziona cartella output", initialdir=Path.cwd())
        if p:
            self.output_dir = Path(p)
            self.out_lbl.config(text=f"Output: {self.output_dir}", fg='green')

    def _run(self):
        if not self.selected_file:
            messagebox.showerror("Errore", "Seleziona un file prima di eseguire")
            return
        if self.running:
            messagebox.showwarning("Attenzione", "Esecuzione già in corso")
            return

        self.running = True
        self.run_btn.config(state='disabled')
        self.progress_lbl.config(text="Esecuzione...", fg='orange')
    ...
    def _worker(self):
        try:
            ...
            if p.suffix.lower() == '.json':
                problems = load_problems_from_json(p)
            else:
                problems = load_problems_from_csv(p)

            if not problems:
                self._ui_error("Nessun problema caricato dal file")
                return

            runner = ModelRunner(problems)
            results = runner.run_all()

            out_dir = self.output_dir or Path('reports')
            out_dir.mkdir(parents=True, exist_ok=True)

            ts = datetime.now().strftime('%Y%m%d_%H%M%S')
            html_path = out_dir / f"comparison_report_{ts}.html"
            json_path = out_dir / f"comparison_report_{ts}.json"

            HTMLReportGenerator(results).generate(html_path)
            json_path.write_text(json.dumps(results, indent=2), encoding='utf-8')

            self._ui_success(f"Report generati:\n- {html_path}\n- {json_path}")
    ...
    def _reset(self):
        ...
        self.progress_lbl.config(text="Pronto", fg='green')
        self.run_btn.config(state='normal')
        self.running = False
...
def main():
    print("=" * 60)
    print("🚀 Gurobi vs QAOA Comparator (GUI)")
    print("=" * 60)
    App().run()
